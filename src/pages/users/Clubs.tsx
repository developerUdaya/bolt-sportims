...
const Clubs: React.FC = () => {
  const [clubs, setClubs] = React.useState<Club[]>(mockClubs.filter(c => c.approved));
  ...
  const handleSaveClub = (clubData: Partial<Club>) => {
    if (modalMode === 'create') {
      const newClub: Club = {
        id: `${Date.now()}`,
        ...clubData
      ...
      setClubs(prev => prev.map(c => 
        c.id === selectedClub.id ? { ...c, ...clubData } : c
      ));
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage registered clubs</p>
        </div>
